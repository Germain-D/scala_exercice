...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] = (x, l) match {
    case (_, Nil ) => None
    case (0,head::_) => Some(l.head)
    //case k if k > 0 => findKth(k - 1, l.tail)
    case(x,head::tail) => nthA(x -1, l.tail)  }
  ...
  def reverseA[T](list: List[T]): List[T] = { list.foldLeft(List[T]())((x,y) => y :: x) }
  ...
  def average(values: Iterator[Double]): Option[Double] = {
    ...
    val v1 = values.map(x => (x,1))
    val res = v1.reduce((a,b) => ( a._1 + b._1, a._2 + b._2 ))
    Some(res._1/res._2)
    }
  }
